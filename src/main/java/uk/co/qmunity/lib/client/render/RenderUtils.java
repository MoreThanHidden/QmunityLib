...
public class RenderUtils {
    ...
    public static final void drawGradientRect(int x1, int y1, int x2, int y2, int c1, int c2) {
        ...
        buff.color(f1, f2, f3, f);
        buff.pos(x2, y1, zLevel).endVertex();
        buff.pos(x1, y1, zLevel).endVertex();
        buff.color(f5, f6, f7, f4);
        buff.pos(x1, y2, zLevel).endVertex();
        buff.pos(x2, y2, zLevel).endVertex();
        tessellator.draw();
    ...
    public static void renderItem(ItemStack item) {
        ...
        EntityItem ghostEntityItem = new EntityItem(Minecraft.getMinecraft().world);
        ghostEntityItem.hoverStart = 0.0F;
        ghostEntityItem.setEntityItemStack(item);
        GL11.glColor3d(1, 1, 1);
        if (item.getItem() instanceof ItemBlock) {
            ItemBlock testItem = (ItemBlock) item.getItem();
            Block testBlock = testItem.block;
            if (customRenderItem.shouldRenderItemIn3D(new ItemStack(testBlock))) {
                GL11.glScaled(1.2, 1.2, 1.2);
            }
        }
        Minecraft.getMinecraft().getRenderManager().doRenderEntity(ghostEntityItem, 0, 0, 0, 0, 0, false);
    }

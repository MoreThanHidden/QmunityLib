...
public class SystemInfoHelper {
    ...
    public static List<String> getTPSSummary() {
        ...
        for (WorldServer world : FMLCommonHandler.instance().getMinecraftServerInstance().worlds) {
            ...
            //todo Re-add the Dimension Names
            textOutput.add("[" + world.provider.getDimension() + "]" + ": " + timeFormatter.format(getWorldTickTime(world)) + "ms [" + timeFormatter.format(getDimensionTPS(world))
                           + "]");
        }
    ...
    public static List<String> getTPSDetail(int dimension) {
        ...
        //todo Re-add the Dimension Names
        textOutput.add("Information for [" + dimension + "]");
    ...
    private static String getPlayersForDimension(int dimension) {
        ...
        if (players.size() == 0) {
            ...
            while (ite.hasNext()) {
                playersString = playersString + ite.next().getCommandSenderEntity();
                if (ite.hasNext()) {
